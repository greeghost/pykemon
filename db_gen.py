...
def sql_connection(gen):
    try:
        ...
        # print('Connection to ' + gen + '.db successfully established.')
        return con
    except Error:
        print(Error)
...
def type_table(con):
    cursorObj = con.cursor()
    try:
        cursorObj.execute("CREATE TABLE type_effectiveness(id integer PRIMARY KEY AUTOINCREMENT, def text, off text, effectiveness float)")
        # print('Table successfully created')
        con.commit()
    except Error as e:
        # print('Error:', e)
...
def pk_insert(con, row):
    ...
    A = row.get('abilities')
    A.sort()
    entities = [row.get('oob')['dex_number'], row.get('name'), T[0], T[1] if len(T) > 1 else None, row.get('hp'), row.get('atk'), row.get('def'), row.get('spa'), row.get('spd'), row.get('spe'), A[0], A[1] if len(A) > 1 else None, A[2] if len(A) > 2 else None, row.get('formats')[0] if len(row.get('formats')) > 0 else None]
...
def type_insert(con, row):
    ...
    name = row.get('name')
    for D in E:
        cursorObj.execute('''INSERT INTO type_effectiveness(def, off, effectiveness) VALUES(?, ?, ?)''', [D[0], name, D[1]])
    con.commit()
...
if __name__ == '__main__':
    ...
    poke_data = data.get('pokemon', []) # Autres champs : formats, natures, abilities, moves, types(name, atk_effectives, genfamily, description), items
